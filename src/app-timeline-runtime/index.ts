...
export function createTimelineRuntime(options: CreateTimelineRuntimeOptions) {
  const { root, state, refresh, queueFramePanelScroll, setStatus } = options;
  return createTimelineSortableSystem({
    root,
    onMoveAction: (actionId, targetFrameId, insertIndex) => {
      if (isNewFrameDropzoneId(targetFrameId)) {
        const next = moveActionToNewFrame(
          state.frames,
          actionId,
          readNewFrameDropzoneInsertIndex(targetFrameId) ?? insertIndex
        );
        ...
        selectActionInFrame(
          ...
          actionId,
          next.newFrameId,
          action?.type === "annotation" ? action.annotationId : null
        );
        queueFramePanelScroll(next.newFrameId);
        refresh();
        setStatus("动作已移入新建帧");
        return;
      }
